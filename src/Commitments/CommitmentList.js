...
const List = ({nap, counter, data: { title: mainTitle, commitments }, index, total }) => {
  return (
...
const CommitmentList = ({ loading, napData, nap }) => {
  ...
  <h6 className="text-lg font-semibold text-text-dark font-sans tracking-open">{nap === 'nap1' ? 'NAP 1': 'NAP 2'}</h6>
</div>
          {loading && [1,2,3].map(() => Skeletaloader)}
          {!loading && napData.map((data, index) => (
            <List index={index} total={napData.length - 1} nap={nap} counter={counter} data={data} />
          ))}
        </div>
      </div>
