...
//import withRouter from '../commitment/milestones';
...
fetchCommitmentFn(id) {
    axios.get(`${customConfig.API_URL}/read-news/`+id)
    .then(res => {
      const news = res.data;
      this.setState({ news },()=> {
        console.log('them', this.state.news)
      } );
    })
}
    ...
    return (
      <div>
        ...
        <div className="main-blogpic" style={{height:"400px", overflow:"hidden"}}>
            <img src={this.state.news?Parser(this.state.news.data.banner):''} alt='' width="100%" style={{marginTop:"-6em"}} />
            <div className="absol">
                <p className="c-blog-details-h4 text-center">  {this.state.news?this.state.news.data.title:''}</p>
...
<div className="py-5"></div>
                  {this.state.news?Parser(this.state.news.data.body):''}
